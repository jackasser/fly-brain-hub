...
**文章で説明せず、図と対比で読ませる。** 長い段落を並べない（2026-09-14 ユーザー指示「文字が多くてわからない」）。
内容は次の 4 つをこの順で持ち、両ロケールで同じ構成にする。

1. **「測った部分」と「決めた部分」の区別。** 配線図は解剖であって生理ではない。この関係
   （測った配線図 ＋ 人が決めたダイナミクスのモデル ＝ 動く「ハエの脳」）を**インライン SVG の図 1 枚**で示す。
   代表的なデータセットはカードで、配線図に入っていない項目は定義リストで出す。
2. **できること / できないこと。** 段落ではなく**左右 2 列の対比**にする。各項目は見出し 1 行と補足 1 行。
   できる側は実時間より速いこと・身体につながること・配線が計算資源になること・回路を検証できること。
   できない側は学習しないこと・エミュレーションではないこと・見出しの数と回す数が違うこと・書き換えが未解決なこと。
...
| AC-18-7 | 本文に図（`<svg>`）と左右対比（`.compare`）があり、本文のテキスト量が JA 1,400 字・EN 2,600 字を超えない | `dist.test.ts` |
