...
5. 初期公開は 30〜40 エントリ。2026-09 の MaleCNS 論文公開以降は事例が急増しているため件数の上限は設けず、
...
| date | string | ✔ | `YYYY` / `YYYY-MM` / `YYYY-MM-DD`。資料の公開・改訂日。ソフトの公開日が確定しない場合は公式リポジトリ作成年までの精度とし、調査日で代用しない |
...
| AC-01-9 | MaleCNS v1.0 の日付は 2026-06-08。説明では 2026-09-03 の論文公開と区別し、公式発表を出典に含む | `data.test.ts` |
...
- 2026-09-16 追加指示：用途の概要カードに自作の SVG 図解（配線・発火・身体・チェス）を置く。各用途の詳細には「入力 → 処理 → 結果」の短い図解と、常時表示する番号付きの開始手順を 3 段階で置く。各手順に操作と確認する結果を示し、入口になる掲載プロジェクトへリンクする。ブラウザのみで始める用途と Python の準備が要る用途を明示する。
- 図は実データや実際の画面の再現ではなく、仕組みを伝える模式図として明示する。装飾色だけに意味を持たせず、テキストと SVG のアクセシブルな名前でも説明する。長い機能解説は折りたたんでも、始め方は隠さない。
- 構成の比較：脳→ゲーム、脳のみ、身体＋人工制御器、脳＋物理身体の閉ループ、脳→実機ロボットの 5 パターンを、脳・身体・接続方法の短い説明と矢印図で比較する。MuJoCo は身体側を担当し、脳との変換処理が必要であること、身体付きでも歩行パターンなどを別に用意する例（Fly.exe）があることを説明する。
- 学習の比較：「学習なし」「読み出し部分を学習」「身体の制御器を学習」を図で示し、どの部分が訓練対象かを文字でも明示する。全プロジェクトの学習方式をこの 3 種に限定する分類ではない。
- 掲載例では脳の出力をゲーム操作などへ変換する構成が目立つ、と傾向を述べる。手法別の全件集計は行っていない旨を併記し、割合・順位・全分野の多数派を断定しない。代表例と一次ソースの対応を解説データに保持し、本文のリンクは同言語の掲載詳細へ向ける。
...
| AC-18-10 | 両言語で用途別の図 4 種にアクセシブルな名前があり、概要カードと用途ごとの模式図・入出力が描画される | `guide-visual.test.ts` / `dist.test.ts` |
| AC-18-11 | 各用途に、準備条件、操作と確認結果を持つ常時表示の 3 ステップ、実在する同言語のプロジェクトへの開始リンクがある。JS 無効のブラウザでも用途から手順へ進める | `explainer.test.ts` / `dist.test.ts` / `e2e/explainer.spec.ts` |
| AC-18-12 | 両言語に構成 5 種と学習の例 3 種を描画し、閉ループの感覚フィードバックと訓練対象を表示する。代表例の出典が sourceRefs に存在し、本文に集計範囲の注記がある | `guide-patterns.test.ts` / `dist.test.ts` |
| AC-18-13 | 両言語の幅 639 / 640 / 700 / 760 / 768px で構成・学習の比較リンクから移動した際、補助ラベルと見出しが固定ヘッダーに隠れない | `e2e/explainer.spec.ts` |
...
## R-21 スクロール連動の3Dハエ（2026-09-17 ユーザー指示）

- 両言語のトップのヒーローに、自作の模式的な3Dショウジョウバエを表示する。赤い複眼、頭・胸・腹、2枚の翅、6本の脚を持ち、実測モデルではない旨を表示する。
- ヒーローをスクロールすると姿勢・位置・翅の開きが連動し、戻すと同じ姿勢に戻る。自動で動き続けず、表示範囲外では描画を止める。
- reduced-motion では静止。JS無効・WebGL非対応・読み込み失敗時は既存SVGを残す。本文・検索操作を妨げず、モバイルでも横にはみ出さない。
- 3Dコードはトップだけで遅延ロードし、描画解像度を制限する。R-12 のSVGはフォールバックとして維持する。

| AC | Given / When / Then | テスト |
|---|---|---|
| AC-21-1 | 両言語で3D領域、模式図の注記、SVGフォールバックを描画する | `fly-scroll.test.ts` |
| AC-21-2 | スクロールで3Dの描画が変わり、戻すと復元する | `e2e/fly-scroll.spec.ts` |
